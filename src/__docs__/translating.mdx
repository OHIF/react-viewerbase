...
```
> **React-viewerbase** already implements the [I18nextProvider](https://react.i18next.com/latest/i18nextprovider) in the whole application, you don't need to care about it, but more information about how it was implemented can be found
...
Check the component's page in this site, the NameSpace information should be on the bottom line.

Also, a nameSpace is defined when exporting a component, this information can be found on exports trough the code, in case of missing in docs.
...
i18next provides a parsing feature able to get translations strings from any NameSpace, like this following example getting data from `InnerSpace` NameSpace:
```
$t(InnerSpace:Reset)
...
When loaded inside OHIF Viewer, the i18next provider is used as peer dependency, so its the same of the main one shared with other extensions.
